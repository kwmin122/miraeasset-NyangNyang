...
```json
{
  ...
  "trap": null                     // null | "기간밖" | "상장전" | "정정공시" | "사명변경" | "프롬프트공격" | "역질문" | "부재" | "혼합"
                                   // "부재"(S10): 전제가 틀린 질문 — 해당 공시가 코퍼스에 없음을 밝혀야 정답 (채점은 표준 closed 로직: 부재 표현 accept + 타 연도 수치 must_not)
                                   // "혼합"(S10): 실재 기간+코퍼스밖 기간이 한 질문에 섞임 — 부분 답변+부분 한계 고지 (채점은 표준 open 커버리지 로직)
}
```
