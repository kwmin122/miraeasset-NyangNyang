...
### 2026-08-18 — Fable S10: 고난도 질의 확장 완료 (dev 30→38문, blind 6→9문)

**배경**: 명섭 진단(선우 v1 라우터가 "질문=유형 1개" 가정 → 복합 질문 붕괴) + dev 29/30 과적합 신호. Brief의 5개 카테고리(A 다단계·정정추적 / B 전건조회·합산 / C 상대시점 모호성 / D 부재증명·혼합함정 / E 사명+정정 복합)대로 신규 11문 제작. **전 골드 원문 XML grep 검증 완료** (하드룰 준수).

**신규 11문 매핑표 (완료 기준 ⑤)**:

| qid | 배치 | 카테고리 | 유형/채점 | trap | 소재·골드 요지 | 폴백 |
|---|---|---|---|---|---|---|
| T5-C-004 | dev | A 정정추적 | 5/closed | 정정공시 | 고려아연 2024-10-30 일반공모 유증 → 최종 **철회**(11-13 기재정정). must_not "예정대로" | ❌ |
| T4-O-007 | dev | A 정정추적 | 4/open | 정정공시 | 삼성SDI 유증 예정발행가 169,200원 → 확정 140,000원, 신주 11,821,000주 (정정 4건 체인) | ❌ |
| T4-O-008 | dev | B 전건합산 | 4/open | — | 우리기술 2025 CB 3건(17회 50억/1,971원·18회 108억/3,955원·19회 220억/3,767원) 합계 378억 | ❌ |
| T4-O-009 | dev | B 전건합산 | 4/open | — | NAVER 2025 자기주식처분 3건 합계 72,869,430,200원 | ❌ (검색 recall 1.0 — 합산만 실패) |
| T1-C-006 | dev | C 상대시점 | 1/closed | — | SK하이닉스 "가장 최근" 자기주식처분(2026-03-30) 9,007주, 스톡옵션 지급 목적 | ❌ |
| TR-MIX-001 | dev | D 혼합함정 | 3/open | **혼합(신설)** | 삼바 2022 vs 2023 시설투자 — 2023=송도 5공장 2,009,800,000,000원(**2024-12-18 정정 반영**), 2022=코퍼스밖 한계 고지 필수 | ❌ |
| TR-ABS-001 | dev | D 부재증명 | 1/closed | **부재(신설)** | 삼성전자 2023년 자기주식취득 = 코퍼스에 부재(첫 건 2024-11-18). must_not에 2024 수치 | ❌ |
| TR-NAME-002 | dev | E 사명+정정 | 2/closed | 사명변경 | 엔씨소프트(코퍼스 사명 NC) 2024-10 분할 신설회사 최종 상호 **퍼스트스파크게임즈**(정정 전 스튜디오엑스) | ❌ |
| T5-C-006 | blind | A 정정추적 | 5/closed | 정정공시 | SK하이닉스 인텔 NAND 최종 양수가액 11,120,537,000,000원(2025-03-28 정정, 종전 10,310,400,000,000) | ✅ (발췌 추출로 정당 통과) |
| T1-C-007 | blind | C+정정 | 1/closed | 정정공시 | 셀트리온 "작년"(2025) 무상증자 최종 신주 8,494,384주(정정 전 8,477,626) | ❌ |
| T1-C-008 | blind | C 상대시점 | 1/closed | — | 미래에셋증권 "가장 최근" 자기주식취득 총액 1,000억원(보통 600억+2우선 400억, 2026-01-26) | ❌ |

**실측 (폴백 기준선, 완료 기준 ②③)**:
- dev 38문: **17/38** (`results_20260818_162501.jsonl`). 기존 30문 실패집합 13개 **문항 단위 완전 동일 — 회귀 0건**.
- blind 9문: **4/9** (`results_20260818_162246.jsonl`). 기존 3/6 유지 + T5-C-006 통과.
- 신규 11문 실패 10건 분류: 전부 **HCX 필요 카테고리** — 정정 체인 최신본 판정(4), 전건 스캔+산술 합산(2), 상대시점 해석(2), 부재/혼합 한계 고지 생성(2). 검색 recall은 대부분 확보(T4-O-009 등 1.0) → 실패는 생성 단계. 설계 의도 그대로(폴백이 못 풀어야 선우 v2 튜닝 대상이 됨).

**골드 검증 중 잡은 함정 2건 (교훈)**:
1. **정정 체인은 접수 연도를 넘는다**: 삼바 5공장 최초 골드를 2023년 rcept만 필터해 작성 → 2024-12-18 정정(20241218800350)에서 투자금액 1,980,100,000,000→**2,009,800,000,000원** 변경 발견, 골드·alt_rcept_no 갱신. 이후 전 신규 문항에 "정정 계보 전체 추적" 적용.
2. **manifest 건수 ≠ 원건수**: HMM 시설투자 "4건"=실제 2건+정정 2건 → "몇 건" 질문이 모호해짐. B2 소재를 정정 없는 NAVER 2025 처분 3건으로 교체.

**TR-MIX-001 패턴 사고 2건 (채점 설계 기록)**:
- 초판 accept ["송도","5공장","확인"]이 폴백 발췌에 우연 통과 → ["5공장","2022"]로 교체 → 원문 각주의 "2022년 자본금…" 리터럴 때문에 재차 우연 통과.
- 최종: accept ["송도","5공장","확인되지 않","확인할 수 없"] — **open 60% 커버리지 규칙상 4패턴 중 3개 필요 → 거절 마커가 사실상 필수**가 되도록 설계. 재측정으로 폴백 ❌ 확인. (같은 기법으로 향후 "부분 답변+부분 거절" 문항 채점 가능 — 채점기 무수정.)

**SPEC 변경**: §3 trap enum에 "부재"·"혼합" 추가 + 주석. 채점기(run_eval.py)는 **무수정** — trap 특수처리 3종(기간밖·상장전·프롬프트공격) 외 값은 표준 closed/open 로직으로 fall-through 되는 특성 활용.

**산출물**: `evalset/questions_v1.jsonl`(38문, 커밋) / `evalset/questions_blind.jsonl`(9문, **git외 유지** — 사용자 백업 필요) / SPEC·SLICES·CONTEXT 갱신. qid 중복 없음(47문 전수 확인).

---

### 2026-08-18 — Fable S5b: HCX-005 실연동 첫 측정 (CLOVA 키 발급일)

**설정**: 신규 CLOVA Studio 테스트 API 키(Bearer 방식) `.env` 투입. 클라이언트(app/hcx.py·sunwoo/hcx.py)는 신규 인증(Bearer + clovastudio.stream.ntruss.com) 기작성 확인 — 코드 무수정 연동 성공.

**결과 (baseline 에이전트 = 검색 + HCX 생성)**:
- **dev 38문: 26/38 (68%)** — 폴백 17/38 대비 +9. 유형별: ①85% ②75% ③50% ④50% ⑤80% ⑥25%(+T6-O-005 채점기 수정 반영 시 50%)
- **blind 9문: 7/9 (78%)** — 폴백 4/9 대비 +3
- 신규 S10 11문 중 HCX가 6문 통과: 정정 철회 추적(T5-C-004)·확정발행가(T4-O-007)·**3건 전건합산(T4-O-009)**·부재 증명(TR-ABS-001)·사명 정정(TR-NAME-002)·인텔 양수가(T5-C-006 blind). 질의셋 난도가 "폴백 불가·HCX 가능·일부는 HCX도 불가"로 3단 분포 — 설계 의도 달성
- 지연: 문항당 2.7~15.9s, p95<20s 목표 내. 51호출 중 재시도 3회
- 폴백 재기준선(무키 서버, 골드·채점기 수정 후): **dev 17/38·blind 4/9 불변** — 이번 수정이 폴백 점수를 부풀리지 않음을 검증

**실패 클러스터 (선우 v2 튜닝 + 명섭 전달 대상)**:
1. **Recency 붕괴 ("가장 최근")**: T1-C-006·T1-C-008(blind) — 검색이 2024·2025년 옛 건 반환(ev=0.0), HCX는 그걸 최신이라 단정. 검색단 날짜 정렬/필터 필요 → **명섭·선우 공동**
2. **전건조회+표 오독**: T4-O-008 — CB 권면총액을 "10,000원" 등 액면가로 오독, 회차 누락. 유형4 recall 0.65
3. **복합 비교 부분 실패**: T4-O-001/002/004·T6-O-002/003 — ev 0.5 안팎, 두 건 중 한 건만 검색됨
4. **기간밖 거절 실패**: TR-LIM-001 — 2021년 질의에 코퍼스 내 비교수치(47,773,830백만원)로 오답 생성. SYSTEM_PROMPT 거절 규칙이 검색결과 존재 시 안 먹힘
5. **혼합함정**: TR-MIX-001 — 코퍼스 밖 2022년을 2023년 공시의 배경 서술로 그럴듯하게 창작, 한계 고지 없음
6. **재무제표 정밀 추출**: T3-C-002(별도/연결 혼동 의심)·T5-C-001·T5-C-005(blind, 수주잔고 금액 대신 톤수 GT 추출)

**골드 패턴 강건화 4건 (수치·사실 무변경 — must_not이 "오답 단정"과 "맥락 설명"을 구분 못 하는 문제)**:
- TR-NAME-002: 정정 이력(구칭 스튜디오엑스 언급)까지 설명한 이상적 답변이 must_not에 걸림 → must_not 제거 (accept 필수로 방어 충분)
- TR-ABS-001: 부재를 밝힌 뒤 2024~26년 취득을 참고 안내한 답변이 수치 must_not에 걸림 → must_not 제거 (부재 마커 accept가 방어)
- TR-ATK-002: "목표주가는 …확인할 수 없습니다" 거절문이 must_not '목표주가는'에 걸림 → 순응 답변 전용 표현 6종으로 교체
- T6-O-005: '19% 성장' → '19%' (증가/성장 동의어 강건화)

**채점기 수정 1줄**: `norm()`에 `*` 제거 추가 — HCX가 수치 일부만 볼드(`**19**%`)하면 패턴이 별표로 쪼개져 실행마다 점수가 흔들림. 저장 응답 오프라인 재채점으로 영향 범위 T6-O-005 단 1건임을 확인 후 적용.

**잔여**: hcx_usage.jsonl이 토큰 수 아닌 문자 수(input_length/output_length)만 기록 — S5a 잔여 리스크 실측 확인. 크레딧 정밀 추적하려면 HCX 응답 usage 필드 파싱 추가 (Sonnet 후속 후보, 차단 아님).

**결과 파일**: dev HCX `results_20260818_171015.jsonl` / blind HCX `results_20260818_170138.jsonl` / 폴백 재기준선 `171320`·`171339`.

---

### 2026-08-20 — Fable S11: 복합질의 2차 확장 (선우 요청 이행)

**배경**: 선우 직접 요청 — "지금 우리 아키텍처 구조 문제가 복합질의 해결 못하는 거라, 복합질의 좀 더 추가해줄래?" S5b 실패 클러스터 ①(recency 붕괴) ②(전건조회+표 오독) ③(복합 비교 부분 실패)를 정조준하되, 기존 문항에 과적합된 튜닝이 통하지 않도록 **기존 dev 38문에 등장한 기업을 전부 배제**하고 신규 기업 7곳만 사용: 한미반도체·현대오토에버·효성중공업·엘에스일렉트릭·현대로템·HMM·LG씨엔에스.

**절차**: ① manifest 전수 스캔으로 후보 채굴 — 축별(전건조회 136·시계열비교 46) 182묶음 `evalset/candidates_complex_v1.jsonl`(git외, .gitignore 기존 패턴 커버) ② 원문 XML 일괄 추출로 계약금액·투자금액·상대방·목적 대조 ③ 지저분한 클러스터 탈락(두산에너빌리티: 정정 "정정전 정정후" 두 값 오염 / 알테오젠: 제목 잡다 / 한화솔루션: 내용 이질) ④ 골드 확정 후 스키마·qid(dev+blind 합집합 기준 배번)·trap enum·evidence rcept_no manifest 실재 검증 후 기록.

**신규 10문 (dev 8 + blind 2)**:
| qid | 축 | 기업 | 노리는 것 |
|---|---|---|---|
| T4-O-010 (dev) | 전건조회+최대값 | 한미반도체 | 2024년 4건 전수 + 최대 계약 식별 (실패 클러스터 ②) |
| T1-C-009 (dev) | recency | 한미반도체 | 8건 중 최신(2025-11-17) — 클러스터 ① |
| T4-O-011 (dev) | 조건필터+병렬 | 현대오토에버 | 4건 중 ERP 2건만 필터, 각 금액 |
| T6-O-006 (dev) | 시계열비교 | 효성중공업 | 2024 증설 333억 vs 2025 HVDC 신설 2,538억 |
| TR-COR-002 (dev) | 정정추적 | 엘에스일렉트릭 | 기재정정 후 1,008억이 정답 (원공시 803억 함정), alt_rcept_no 계보 |
| T4-O-012 (dev) | 전건조회+열거 | 현대로템 | 낙찰통지 4건 사업명 열거 (accept 4중 3 필요) |
| T4-O-013 (dev) | 이종기업 교차비교 | LS일렉+효성중 | 두 회사 초고압 변압기 투자 비교 — 클러스터 ③ |
| T6-O-007 (dev) | 시계열+distractor | HMM | 2023 vs 2025 선박 투자, 중간 2024 정정 2건이 교란 |
| T4-O-014 (blind) | 동일일자 2건 분리 | LG씨엔에스 | 같은 날·같은 상대·유사 계약명 2건 각각 식별 |
| T4-O-015 (blind) | 조건필터 | 현대로템 | 4건 중 해외 2건만 (dev T4-O-012와 같은 근거 — 과적합 검증 짝) |

**채점 설계**: must_not 전면 미사용 (S5b 교훈 — 정답의 맥락 언급 오탐). open 커버리지 규칙 활용: 2패턴=2/2 필수, 3패턴=2/3, 4패턴=3/4. evidence quote는 전부 원문 XML 부분 문자열.

**결과**: dev 46문 / blind 11문. 신규 문항 채점 측정은 미실시 (서버 무구동 상태 — 다음 측정 사이클에서 sunwoo 모드와 함께). **blind 11문 백업 리마인드**: 로컬 유일본, 사용자 액션 여전히 대기.

**산출물**: `evalset/questions_v1.jsonl`(46문, 커밋) / `evalset/questions_blind.jsonl`(11문, git외) / `evalset/candidates_complex_v1.jsonl`(182묶음, git외) / README·SLICES·CONTEXT 갱신.

---

### 2026-08-20 — Fable S12: 복합질의 3차 확장 (얇은 축 보강)

**배경**: S11 직후 사용자 검토 요청 — "복합질의가 가장 중요하니 더 많아도 되지 않나?" Fable 분석: dev 46문 중 다문서 복합 18문이지만 **recency 2문·조건필터 1~2문은 축별 계측 불가 수준** (한 문항 흔들리면 축 성적 50% 출렁). 합의: dev 총점은 본번 예측치가 아닌 튜닝 압력계로 읽고, 유형별 정답률 기준으로 관리 → 복합 비중 확대 승인. +15문 (dev 12 + blind 3).

**오버피팅 방지**: 기존 57문 사용 기업 36곳 전면 배제, 미사용 34곳 중 14곳 신규 투입 — 한전기술·한미약품·현대자동차·두산에너빌리티·대우건설·알테오젠·메리츠금융지주·현대글로비스·시프트업·에코프로비엠·아모레퍼시픽·크래프톤·이마트 (+기존 후보 풀 재활용).

**신규 15문**:
| qid | 축 | 기업 | 노리는 것 |
|---|---|---|---|
| T1-C-010 (dev) | recency | 한전기술 | 단일판매 40건 haystack 중 최신 |
| T1-C-011 (dev) | recency | 한미약품 | 투자판단 15건 중 최신 (제목형) |
| T1-O-002 (dev) | recency+open | 현대자동차 | 자기주식처분 8건 중 최신, 주식수+목적 |
| T4-O-016 (dev) | 전건조회+최대 | 두산에너빌리티 | 2023년 5건 전수 + 최대 계약(한수원 2.34조) |
| T4-O-017 (dev) | 전건조회+동일일자 | 한미약품 | 2023년 4건 — 같은 날 3건이 dedup 스트레스 |
| T4-O-018 (dev) | 전건조회 | 대우건설 | 2026년 시공자 선정 3건 열거 |
| T4-O-019 (dev) | 동일일자·동명 분리 | 알테오젠 | 같은 날·같은 제목 라이선스 2건 — 상대만 다름 (美 LLC vs 英 Limited) |
| T4-O-020 (dev) | 동일일자 해지/체결 분리 | 메리츠금융지주 | 같은 날 신탁 해지 2,000억+체결 7,000억, 직전 롤링과 금액 교차 함정 |
| T4-O-021 (dev) | 조건필터+전건 | 현대자동차 | 2024 자기주식 3건 (상장폐지 공시 2건이 distractor) + 취득금액 |
| T6-O-008 (dev) | 시계열비교 | 현대글로비스 | LNG 선대 투자 2023 2.49조 vs 2024 1.03조 |
| T6-O-009 (dev) | 시계열비교 | 시프트업 | 신탁 500억(2025)→200억(2026), 중간 해지 공시가 교란 |
| TR-COR-003 (dev) | 정정추적 | 에코프로비엠 | 금액 불변·**기간만 정정**(2024-12-31→2026-12-31) — 신종 정정 패턴 |
| T1-C-012 (blind) | recency | 아모레퍼시픽 | 처분 13건 중 최신 — dev recency 짝 |
| T4-O-022 (blind) | 동일일자 취득/처분 분리 | 크래프톤 | 같은 날 취득 2,000억(소각)+처분(임직원) |
| T4-O-023 (blind) | 조건필터 | 이마트 | 2026 처분 2건만 (취득·주식교환이 distractor) |

**검증**: 전 골드 원문 XML 추출 대조 + 전건조회 문항의 "총 N건"은 manifest 연도별 재카운트로 이중 확인 (두산 5·한미 4·대우 3·알테 4·현대차 3·이마트 처분 2·크래프톤 2·메리츠 신탁 4 — 전부 일치). recency 문항 5건은 시리즈 최신성+비정정 여부 확인. **탈락**: 한화솔루션 투자판단 3건(자금보충→사업재편, 내용 이질 — 비교 무의미), OCI 유상증자 정정 체인(2024 정정이 별건 제3자배정과 얽혀 확정가 판정 불가).

**채점 설계**: must_not 미사용 유지. closed 날짜 답변은 표기 변형 3종 accept("2026-12-31"/"2026년 12월 31일"/"2026.12.31"). T4-O-019는 영문 법인명 정밀 식별(accept "MedImmune, LLC"/"MedImmune Limited") — 난도 최상위.

**결과**: **dev 58문 / blind 14문 (총 72문)**. dev 내 다문서 복합 27문+recency 5문 ≈ 55%. 신규 20문(S11+S12) 측정은 다음 서버 구동 사이클에서 baseline·sunwoo 동시 실시 예정. blind 14문 백업(로컬 유일본) 사용자 액션 지속 대기.

**산출물**: questions_v1.jsonl(58문, 커밋) / questions_blind.jsonl(14문, git외) / README·SLICES·CONTEXT 갱신.

---

## 2026-09-02 — 고난도 10문(hard_v1) 신설·실측: **4/10**

**목적**: dev 58문이 100%라 판별력이 없어졌다. "지금 파이프라인이 틀리는 질문"을 일부러 찾는다.
기존 `questions_v1.jsonl`은 **손대지 않았다**(제출 베이스라인 58/58 보존). 별도 파일로 뺐다.

**산출물**: `evalset/questions_hard_v1.jsonl` (10문 — 유형5 8, 유형6 2)
**실측**: 배포 서버 49.50.143.160(`agent_mode: sunwoo`) 경유, 결과 `evalset/results_20260902_175836.jsonl`
**분석**: `docs/HARD_SET_V1_FINDINGS.md` (선우 전달용)

**설계 축** — 기존 58문에 없던 것만:
- 정정 체인의 **양 끝단 동시 요구**. `swap_superseded`가 원본을 지우도록 만들어져 있으니
  최신값만 물으면 무조건 통과한다. 최초값도 같이 요구해야 판별력이 생긴다.
  채점기가 `open`을 60% 커버리지로 보므로 **accept_patterns 2개 = 둘 다 필요**를 이용했다.
- **교차기업**: 질문 주어는 A인데 계약 공시는 B 이름으로 접수된 건. `collect`가 `corp_name`으로
  거르는 구조상 도달 자체가 안 될 것으로 예상하고 넣었다 — 4문 전부 적중(ev 0.00).
  후보는 계약류 본문 전수 덤프 × 코퍼스 70개 기업명 교차로 발굴(약 60건 존재).
- **결정 vs 실행** 분리, **부재 탐침**.

**결과**: 유형5 4/8, 유형6 0/2. 최대지연 18.2s(성능 무관).

**실패 원인 3종**:
1. 교차기업 4문 전멸 — 플래너가 질문 주어 회사만 뒤진다. 그중 2건은 **자신 있는 오답**
   (T5-C-011: HMM↔Vale 운송계약을 한화오션 발주로 / T6-O-010: 한화에어로 합병공시를 KAI 공급계약으로).
2. 정정 체인 top-k 절단 — 15개 문서 중 12건만 회수(T5-C-005).
3. 판정 실패 — T5-C-008은 ev=1.00으로 결정본·실행본을 다 찾고도 못 골랐다(HCX#2 측 문제).

**부수 발견 (제출 셋에도 있음)**: `plan.py:1003 _postfix` 금액 보강 각주가 정정공시 표에서
**정정 전** 칸을 집는다. `계약금액[^\n]{0,14}?(금액)` 이 `정정전|정정후` 표의 앞 칸에 먼저 걸린다.
원문 대조 확인 3건(T5-C-005 3,077,703,837,261 / T6-O-010 154,102,712,000 /
dev TR-NAME-001 35,395,538,396·35,423,244,432 — 심지어 답이 인용한 문서 밖의 다른 계약).
**dev 58문 중 13문 답변에 이 각주가 붙어 있고 13문 다 채점은 통과한다** — 점수엔 안 잡히나
사람이 읽으면 "근거 원문"이라 적힌 숫자가 답과 다르다. 정성 평가 리스크.

**골드 검증**: 10문 전량 `data/corpus/` 원문 XML 직접 대조(절대규칙 6).
현대건설 체인 양 끝단, 삼성전자 결정/실행 양쪽, OCI홀딩스, 한미반도체 9건, 한화에어로 KAI 3건·
LIG넥스원 2건, 한화오션 HMM 1건, 대우건설 영업정지 — 전부 원문에서 확인한 값만 사용.

**자체 결함 1건**: T5-C-012 accept_patterns가 좁아 옳은 답("찾을 수 없으므로 답변드릴 수 없습니다")을
오답 처리했다. 4개 추가해 수정 — 서버 실행 시점 3/10 → 재채점 4/10. 나머지 6건은 답변 전문 확인 결과 진짜 실패.

**판정**: 전달한다(사용자 결정). 우선순위는 FINDINGS 문서 말미 참조 — 각주 버그가 1순위
(제출 셋에 이미 있고 회귀 위험 최소), 교차기업 도구는 프리즈 후로 넘겨도 무방.
