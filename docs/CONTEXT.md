...
- **S11 완료 (2026-08-20, 선우 요청 이행)**: 복합질의 2차 확장 — 평가셋 **57문** = dev **46문**(38+신규 8) + blind **11문**(9+신규 2). 신규 10문은 S5b 실패 클러스터 ①②③ 정조준 + **기존 38문 미사용 기업 7곳만 사용(오버피팅 방지)**: 한미반도체·현대오토에버·효성중공업·엘에스일렉트릭·현대로템·HMM·LG씨엔에스. 축: 전건조회+최대값·recency·조건필터·시계열비교·이종기업 교차비교·정정추적(alt_rcept_no). 전부 원문 XML 검증, must_not 미사용(S5b 교훈). 신규 문항 채점 측정은 다음 사이클(서버 무구동). 매핑표 LOG 2026-08-20 S11
- **S12 완료 (2026-08-20, 사용자 결정 — 복합 비중 확대)**: 복합질의 3차 확장 — 평가셋 **72문** = dev **58문**(46+12) + blind **14문**(11+3). 얇은 축 보강: recency 2→8문(한전기술 40건 haystack 등)·전건조회 "총 N건" manifest 재카운트 이중검증·**동일일자 분리 신설**(같은 날 취득/처분·해지/체결·동명 라이선스 2건). 기존 57문 사용 기업 전면 배제, 신규 14곳 투입. dev 총점은 본번 예측치가 아닌 **튜닝 압력계**로 읽기로 합의(유형별 정답률 기준 관리 — dev 내 복합 비중 ≈55%). 매핑표 LOG 2026-08-20 S12
...
- [ ] **blind 평가셋 백업**: `evalset/questions_blind.jsonl`(**14문 — S10 +3, S11 +2, S12 +3**)·`candidates_s4.jsonl`·`candidates_complex_v1.jsonl`은 git외 로컬 유일본 — 안전한 곳(개인 클라우드 등, 선우 접근 불가)에 사본 보관
- [ ] **CLOVA 서비스 앱 신청** (콘솔, 사무국 공식 권고 2026-08-19): 테스트 앱 60 RPM/60,000 TPM → 서비스 앱 300 RPM/180,000 TPM, 과금·크레딧 차감 동일, 요건 충족 시 즉시 승인. 승인 후 **서비스 API 키 발급 → `.env` 교체 (서버 전용)**, 기존 테스트 키는 선우 튜닝용으로 분리 → 서버·튜닝이 rate limit 풀을 나눠 쓰는 문제 해소. 참고: TPM은 실출력이 아니라 input+maxTokens 선언값으로 차감(사무국 확인) — baseline 1콜/문항(maxTokens 1024), sunwoo 모드 2콜/문항(추출 200+생성 700~1400)이라 테스트 한도로는 평가 본번+튜닝 병행 시 429 위험
...
evalset/        questions_v1.jsonl(dev 58문) questions_blind.jsonl(blind 14문, git외) run_eval.py(채점기)
...
~~S1~~ → ~~S2~~ → ~~S3~~ → ~~S6~~(잠정) → ~~S4~~ → ~~S5a~~ → ~~S8 문서분~~ → ~~S9~~ → ~~S10~~ → ~~S5b~~ → ~~S11~~ → ~~S12~~ 완료.
다음: ① 선우에게 **dev 58문**(S11·S12 복합질의 20문 포함) + 실패 클러스터 6종 공유 (v2 튜닝 목표) + 문제 #9(recency) 명섭 전달 ② **baseline·sunwoo 모드로 dev 58문 측정** — 신규 20문 기준선 확보 겸 제출 모드 판단 ③ S7(NCP 서버 생성·배포) — PLAN 주차표상 지금이 착수 시점 ④ S8 리허설 → 9/6 프리즈. blind 14문 백업은 여전히 사용자 액션.
