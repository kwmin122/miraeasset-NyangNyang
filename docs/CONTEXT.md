...
- **S5b 완료 (2026-08-18, CLOVA 키 연동)**: HCX-005 실측 — **dev 26/38 (68%) · blind 7/9 (78%)**, 폴백 대비 +9/+3. 지연 2.7~15.9s (p95<20s 목표 내). 골드 패턴 4건 강건화(must_not 맥락언급 오탐) + 채점기 norm() `*` 제거(HCX 볼드 쪼개짐). 실패 클러스터 6종 분류 완료(recency 붕괴·전건조회 오독·복합비교 부분실패·기간밖 거절실패·혼합함정·재무제표 정밀추출) — 선우 v2 튜닝 목표가 명확해짐. LOG 2026-08-18 S5b
...
| 9 | **"가장 최근" 질의 recency 붕괴** — 검색이 옛 공시 반환(T1-C-006·T1-C-008 ev=0.0), HCX가 그걸 최신으로 단정 | **명섭(검색단 날짜 정렬)+선우(라우팅) 공동** — S5b에서 실증, dev·blind 양쪽 재현 |
| 10 | hcx_usage.jsonl이 토큰 아닌 문자수만 기록 — 크레딧 정밀 추적 불가 | Sonnet 후속 후보 (차단 아님) |
...
- [x] ~~CLOVA Studio API 키 발급~~ — **완료 (2026-08-18)**: 테스트 API 키 → `.env`. S5b 측정 완료. 선우용은 **별도 테스트 키 추가 발급** 권장(유출 시 개별 폐기) — 선우 계정 크레딧 유무 먼저 확인
...
~~S1~~ → ~~S2~~ → ~~S3~~ → ~~S6~~(잠정) → ~~S4~~ → ~~S5a~~ → ~~S8 문서분~~ → ~~S9~~ → ~~S10~~ → ~~S5b~~ 완료.
다음: ① 선우에게 dev 38문 + 실패 클러스터 6종 공유 (v2 튜닝 목표) + 문제 #9(recency) 명섭 전달 ② **sunwoo 모드로 dev 38문 측정** — baseline 26/38이 제출 모드 판단 기준선 ③ S5b 실측 기반 2차 질의 확장(실패 축 조준, 12~15문) ④ S7(NCP 서버)은 8월 말 리허설 직전. blind 9문 백업은 여전히 사용자 액션.
