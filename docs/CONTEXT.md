...
- [ ] **blind 평가셋 백업**: `evalset/questions_blind.jsonl`(**11문 — S10에서 3문, S11에서 2문 추가됨**)·`candidates_s4.jsonl`·`candidates_complex_v1.jsonl`은 git외 로컬 유일본 — 안전한 곳(개인 클라우드 등, 선우 접근 불가)에 사본 보관
...
evalset/        questions_v1.jsonl(dev 46문) questions_blind.jsonl(blind 11문, git외) run_eval.py(채점기)
...
~~S1~~ → ~~S2~~ → ~~S3~~ → ~~S6~~(잠정) → ~~S4~~ → ~~S5a~~ → ~~S8 문서분~~ → ~~S9~~ → ~~S10~~ → ~~S5b~~ → ~~S11~~ 완료.
다음: ① 선우에게 **dev 46문**(S11 신규 8문 포함) + 실패 클러스터 6종 공유 (v2 튜닝 목표) + 문제 #9(recency) 명섭 전달 ② **baseline·sunwoo 모드로 dev 46문 측정** — S11 신규 8문 기준선 확보 겸 제출 모드 판단 ③ S7(NCP 서버 생성·배포) — PLAN 주차표상 지금이 착수 시점 ④ S8 리허설 → 9/6 프리즈. blind 11문 백업은 여전히 사용자 액션.
