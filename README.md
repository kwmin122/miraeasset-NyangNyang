...
- 서버는 4GB 대회 스펙에서 검증 완료, 평가셋 38문 + 자동 채점기 완비. 현재 폴백 기준선 17/38 (HCX 연동 전).
- **선우**: 아래 [에이전트 모드](#에이전트-모드-env의-agent_mode)의 계약대로 `sunwoo_agent.py`를 구현하면 꽂힌다. 완성되면 같은 38문으로 baseline과 성적 대조 → 높은 쪽으로 제출. 튜닝은 `evalset/questions_v1.jsonl`(dev 38문)로만 — **별도 blind 9문이 비공개로 있고(과적합 검증용, 민경욱 보관) 제출 모드 결정 때만 투입된다.** S10에서 고난도 8문 추가됨(다단계 정정추적·전건합산·상대시점·부재/혼합 함정·사명+정정 복합) — v2 튜닝은 이 신규 문항이 핵심.
...
- `evalset/questions_v1.jsonl` — dev 38문, 전부 코퍼스 원문으로 골드 검증 (6유형·8평가기준·함정 7종 커버: 기간밖/상장전/정정공시/사명변경/프롬프트공격/부재/혼합)
- blind 9문은 git·이 파일 목록에 없음 (선우 튜닝 비노출 — 과적합 방지 홀드아웃)
