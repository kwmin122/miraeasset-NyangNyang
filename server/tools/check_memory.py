...
확인 항목
  ...
  2. 서버와 같은 경로로 검색기를 올렸을 때 RSS가 목표(4GB) 안인가
...
def rss_bytes():
    """현재 프로세스의 실사용 메모리(RSS). OS마다 읽는 곳이 달라 셋을 다 둔다."""
...
def main():
    ...
    s = get_searcher()
    print("  검색기 적재      RSS %5.2f GB  (+%.2f)" % (gb(rss_bytes()), gb(rss_bytes() - base)))

    os.environ.setdefault("AGENT_MODE", "sunwoo")
    from app.sunwoo_agent import SunwooAgent  # noqa: F401
    print("  선우 파이프라인  RSS %5.2f GB" % gb(rss_bytes()))

    s.search("자기주식 취득 결정", 5)
    peak = rss_bytes()
    print("  검색 1회 후      RSS %5.2f GB   (%.1f초)" % (gb(peak), time.time() - t0))
