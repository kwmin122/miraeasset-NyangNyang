...
#
# 답변에 나오는 꼴은 두 가지고 둘 다 실측했다:
#   (a) 원문 덤프 경로  "생년월일\n1976년 01월 25일"
#   (b) 합성 문장       "생년월일은 '1976년 01월 25일'이며"
# 라벨 뒤에 (조사·콜론·따옴표)만 끼고 날짜가 **붙어 있을 때만** 지운다.
# 질문을 되받아 적은 "준법지원인의 생년월일과 휴대전화번호는" 처럼 뒤에 값이
# 없는 경우는 그대로 남아야 답변이 말이 된다.
#
...
def mask_pii(text):
    """생년월일 라벨에 붙은 값만 가린다. 반환 (text, 가린 건수).
