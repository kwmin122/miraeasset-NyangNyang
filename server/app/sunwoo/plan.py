...
# 플래너 경로가 답을 못 만들고 파이프라인 폴백(evidence_only_answer)으로 내려올 때
# 붙는 한계 고지 — A6 입구 2 (실측 T2-O-003: 플래너가 topic 없는 find 를 내서
# validate 반려 -> 고지 없는 원문 덤프가 답인 척 나갔다). _DEGRADE_HEAD 의
# "일시적인 시스템 제약"은 429·타임아웃 얘기라 여기엔 안 맞는다 — 반려는 쿼터가
# 아니라 계획 품질 문제다. 원인을 지어내지 않고 한계만 밝힌다.
FALLBACK_HEAD = ("이 질문에 대한 완결된 답변을 생성하지 못했습니다. 대신 수집된 "
                 "공시 근거에서 질문과 관련해 확인된 내용을 원문 표기 그대로 정리합니다.")
